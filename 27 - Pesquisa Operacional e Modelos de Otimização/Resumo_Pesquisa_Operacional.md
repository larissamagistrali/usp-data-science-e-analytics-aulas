...
## 🐍 Nota sobre Implementação Python

Os slides desta aula tratam exclusivamente da **formulação** dos modelos matemáticos (variáveis, função objetivo, restrições). A **resolução** desses modelos (Método Simplex e derivados) é anunciada como tema da "próxima aula". 

Para implementação computacional, podem ser utilizadas bibliotecas como **PuLP** (para Programação Linear/Inteira) ou **scipy.optimize** (para otimização contínua), aplicando os modelos exatamente como formulados nesta aula.
